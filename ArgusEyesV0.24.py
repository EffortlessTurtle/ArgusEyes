#       <ArgusEyes,webscraper for news stories,v0.24,GitHub,EffortlessTurtle>
########################################################################
#       <TODO>
...
def epoch_latest(fo):  # Pulls title, blurb and date written for Epoch - latest
    ...
    #Gets first 3 pages
    for x in range(2, 3):
        response = requests.get(url)  #  must stay here, otherwise you never request the next page's content from website
        text = response.text
        data = BeautifulSoup(text, 'html.parser')  
        # For loop through each container for title, time and who reported
        for tag in data.find_all(class_="article_info text"):
...
def yahoo_fin_RSS(fo): #Pulls yahoo finance RSS
