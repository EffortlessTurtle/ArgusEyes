#       <ArgusEyes,webscraper for news stories,v0.29,GitHub,EffortlessTurtle>
########################################################################
#       <TODO>
# -Check path to ensure that it's always pointed at the Desktop
# -send values from multithreading to lists, then combine those lists, add in the newlines and the write that list to the file, this should fix the multithreading problem
# -Fix newlines in India Today entries
# - finish and release
...
class Quill: #For the write to file section of a FXN
    ...
    def write(title, ex, date,site_nam, fo): #Writes Human readable output to the file from main()
    ...
    def write_2(title, ex, date,site_nam, fo): #Writes Human readable output this to the file from main()
        """ Writes Human Readable file"""
        fo.write(str(date) + "\n\n")
        fo.write(f'{title} \n')
        fo.write("\n")
    ...
    def csv_file_prep(fo): #Preps files as CSV file
        ...
        fo.write(f"URL,HEADLINE,EXCERPT,SOURCE,DATE\n")
        
    def csv_write(title, excerpt,source, date,fo,url): #Writes the entries to CSV
        """ Writes the entries to CSV output is 
        {fo},{title},{excerpt},{source},{str(date)}"""
        fo.write(f"{url},{title},{excerpt},{source},{str(date)}\n")
    

class _Multithreader:
    """Only for the multithreading part"""
    def MTHR (site_list, fo):
        """This is the multi-threaded thing"""
...
def epoch_US(fo):  # Pulls title, blurb and date written for Epoch - US
    ...
    for x in range(2, 3):
        ...
        for tag in data.find_all(class_="article_info text"):
            ...
            Quill.csv_write(title,more_info,site_nam,date,fo,url)
...
def bbc_WOR_RSS(fo):        #Pulls title, blurb and date BBC world
    ...
    Quill.csv_write(p_bold_title,p_bold_more_info,site_name,p_date,fo,url)
    ...
    for entries in parsed_url:
        ...
        Quill.csv_write(title,excerpt,site_name,date,fo,url)
        x+=1
...
def bbc_UK_RSS(fo):        #Pulls title, blurb and date BBC UK
    ...
    for entries in parsed_url:
        ...
        excerpt = parsed_url.entries[x].summary
        date = parsed_url.entries[x].published
        Quill.csv_write(title,excerpt,site_nam,date,fo,url)
        x+=1
...
def Stripes_US_Navy(fo): #Gets US Navy News from stripes
    ...
    date = dt.date.today()
    Quill.csv_write(title,ex,site_nam,date,fo,url)
...
def main():
    ...
    TargFilePath = f'{str(cwd_parent)}/news/{str(dt.date.today())}.txt'
    NewsFolderPath = str(cwd_parent)+"/news"
    #Try-catch to stop the error if the directory isn't found
    try:
        fo = open(TargFilePath, "w")
        print(f'News at: {NewsFolderPath}')
    except:
        os.mkdir(NewsFolderPath)
        fo = open(TargFilePath, "w")
        print(f'News at: {NewsFolderPath}')
    ...
    Stripes_US_Veterans]
    #Keep the sleeps
    _Multithreader.MTHR(site_list1,fo)
    time.sleep(.5)
    _Multithreader.MTHR(site_list2,fo)
    time.sleep(.5)
    _Multithreader.MTHR(site_list3,fo)
    time.sleep(.5)
    _Multithreader.MTHR(site_list4,fo)
    time.sleep(.5)
    _Multithreader.MTHR(site_list5,fo)
    time.sleep(.5)
    _Multithreader.MTHR(site_list6,fo)
    time.sleep(.5)
    _Multithreader.MTHR(site_list7,fo)    
...
# Still having issues with the write stuff getting corrupted, maybe should split the multi-thread
#
#
