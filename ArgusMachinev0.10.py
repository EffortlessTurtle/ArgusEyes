...
#REMOVE BEFORE GOING TO GITHUB!!!

def TEMPLATE_SCRAPE(fo):        #Pulls title, blurb and date -TEMPLATE
    """this pulls the bbc world news
    ++++
    NOTE:
    -this one has very long class names. and is the template of many of its contemporaries.
    ...
    data = BeautifulSoup(text, 'html.parser')
    #Loading bar
    widgets = ['Template: ', progressbar.AnimatedMarker()]
    bar = progressbar.ProgressBar(widgets=widgets).start()
    #Write in title
    Quill.title_write(site_nam,fo)
    #Class name stand-ins (CSI)
    container_CSI = ""  #findall() makes a list, just use main container name
    title_CSI = ""
    excerpt_CSI = ""
    time_CSI = ""
...
def main():
    ...
    #wsj_us(fo) NOTE: WSJ stuff doesn't work right now
    BBC_UK(fo)
    epoch_latest(fo)
    epoch_US(fo)
    epoch_USPol(fo)
